...
def compute_rouge_l(a_gold, a_pred):
...
def calculate_baseline_metrics(target_answer, res_baseline, task_type="qa"):
    ...
    if task_type == "qa":
        ...
        metrics["Baseline_RougeL"] = compute_rouge_l(target_answer, res_baseline)
    return metrics    

def calculate_comprehensive_metrics(base_logits, test_logits, base_attn, test_attn,
                                   base_gen_text, test_gen_text, ground_truth_text, 
                                   tokenizer, task_type="qa", k=50):
    metrics = {}
